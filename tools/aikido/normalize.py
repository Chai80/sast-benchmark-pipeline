...
def _coerce_issues_payload(payload: Any) -> List[Dict[str, Any]]:
    ...
    # Common wrappers seen in Aikido exports.
    for k in (
        "data",
        "issues",
        "findings",
    ...
    # Some payloads nest lists one level deeper (e.g., {"result": {"issues": [...]}}).
    for container_key in ("result", "results", "scan", "scanResult", "scan_result"):
        container = payload.get(container_key)
        if isinstance(container, dict):
            for k in ("data", "issues", "findings", "vulnerabilities"):
                v = container.get(k)
                if isinstance(v, list):
                    return [x for x in v if isinstance(x, dict)]
