...
def run_suite_mode(args: argparse.Namespace, pipeline: SASTBenchmarkPipeline, *, repo_registry: Dict[str, Dict[str, str]]) -> int:
    ...
    suite_def, rc = apply_qa_case_selection(ctx, suite_def)
    if rc is not None:
        return int(rc)

    rc = resolve_suite_run_and_dirs(ctx, suite_def)
    if rc is not None:
        return int(rc)

    maybe_write_replay_file(ctx)

    overall = run_suite_cases(ctx)
    overall = post_run_aggregation_and_qa(ctx, overall)
