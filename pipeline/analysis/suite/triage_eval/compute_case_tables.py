"""pipeline.analysis.suite.triage_eval.compute_case_tables

Helpers to build per-case evaluation tables for triage evaluation.

This module contains the per-case classification and the construction of:
- top-k per-rank rows (triage_topk.csv)
- per-case per-k rows (triage_eval_by_case.csv)

It is split out so the main compute loop in compute_impl stays focused.
"""
...
from .compute_macro_micro import _MacroMicroAgg
